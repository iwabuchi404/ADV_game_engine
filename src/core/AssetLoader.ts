...
class AssetLoader {
  ...
  /**
   * コンストラクタ
   * @param {string} basePath - アセットの基本パス（デフォルトは '/assets'）
   */
  constructor() {
    // 読み込み済みアセットのキャッシュ
    this.cache = {
   ...
   * @returns {Promise<Array<HTMLImageElement>>} 画像要素の配列のプロミス
   */
  preloadCharacterExpressions(characterId: string, expressions: string[]) {
    const paths = expressions.map(
      (expression) => `/images/characters/${characterId}_${expression}.png`
    );
